...
exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/80827d95ecdd671224051.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Yanz X Rams 」', body: `❏ ${date}\n`, sourceUrl: 'https://vt.tiktok.com/ZSdALNCuB/', thumbnail: data }}, options })
    }
